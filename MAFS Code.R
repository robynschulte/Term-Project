...
cat(sprintf('to rise by %1.3f. \n', lm_full_model$coefficients['DrLoganLevkoff']),
    file = out_file_name, append = TRUE)
cat('If the couple is approved by DrPepperSchwartz, housing prices are expected \n',
    file = out_file_name, append = TRUE)
...
##################################################
#
# Age.Difference_Reg gives an example of OLS regression
#   using data imported from a spreadsheet.
#   It automatically generates figures and tables for a
#   pdf document built with LaTeX.
#
...
# Create a table of Dr Pepper Schwartz approval and Married vs Divorced.
out_tab <- table(mafs_data[, 'DrPepperSchwartz'], mafs_data[, 'Married..1..Divorced..0.'])


# Add some column names.
rownames(out_tab) <- c('DrPepperSchwartz')
colnames(out_tab) <- c('other', 'Married..1..Divorced..0.')


# Convert the table to a LaTex table.
out_xtable <- xtable(out_tab[, ],
                     digits = 2, label = 'tab:Married..1..Divorced..0.s',
                     caption = 'Married..1..Divorced..0.')

# Output to TeX file.
tab_file_name <- sprintf('%s/Married..1..Divorced..0.s.tex', tab_dir)
cat(print(out_xtable), file = tab_file_name, append = FALSE)
...
# Calculate a correlation matrix for selected variables.
corr_var_names <- c('Age.Difference', 'DrLoganLevkoff', 'DrPepperSchwartz', 'Married..1..Divorced..0.')
corr_matrix <- cor(mafs_data[, corr_var_names])
print(round(corr_matrix, 3))
...
# Estimate a regression model.
lm_full_model <- lm(data = mafs_data,
                    formula = Age.Difference ~ DrLoganLevkoff + DrPepperSchwartz + Married..1..Divorced..0.)
...
cat('If there was a successful couple approved by Dr Pepper Schwartz, age difference is expected \n',
    file = out_file_name, append = TRUE)
cat(sprintf('to be %1.3f lower. \n', lm_full_model$coefficients['Married..1..Divorced..0.']),
    file = out_file_name, append = TRUE)
...
# Plot the actual house prices against the regression model predictions.
plot(mafs_data[, 'Age.Difference'], mafs_data[, 'predictions'],
     main = 'Regression Model Predictions',
     xlab = 'Age Difference',
     ylab = 'Prediction', pch = 16)
points(mafs_data[mafs_data[, 'DrPepperSchwartz'] == 1, 'Age.Difference'],
       mafs_data[mafs_data[, 'DrPepperSchwartz'] == 1, 'predictions'],
       col = 'green', pch = 16)
points(mafs_data[mafs_data[, 'Married..1..Divorced..0.'] == 1, 'Age.Difference'],
...
# Plot the actual house prices against the regression model predictions.
plot(mafs_data[, 'DrLoganLevkoff'], mafs_data[, 'Age.Difference'],
     main = 'Regression Model Predictions',
     xlab = 'DrLoganLevkoff',
     ylab = 'Age Difference', pch = 16)
points(mafs_data[mafs_data[, 'DrPepperSchwartz'] == 1, 'DrLoganLevkoff'],
       mafs_data[mafs_data[, 'DrPepperSchwartz'] == 1, 'Age.Difference'],
       col = 'green', pch = 16)
points(mafs_data[mafs_data[, 'Married..1..Divorced..0.'] == 1, 'DrLoganLevkoff'],
       mafs_data[mafs_data[, 'Married..1..Divorced..0.'] == 1, 'Age.Difference'],
       col = 'red', pch = 16)
...
beta_DrLoganLevkoff_hat <- coef(lm_full_model)['DrLoganLevkoff']
beta_DrPepperSchwartz_hat <- coef(lm_full_model)['DrPepperSchwartz']
beta_Married..1..Divorced..0._hat <- coef(lm_full_model)['Married..1..Divorced..0.']
...
# Repeat for California without Married..1..Divorced..0.s (green)
reg_line_DrPepperSchwartz <- beta_0_hat +
  beta_DrLoganLevkoff_hat*DrLoganLevkoff_grid +
  beta_DrPepperSchwartz_hat
...
# Repeat for California with Married..1..Divorced..0.s (red).
reg_line_Married..1..Divorced..0. <- beta_0_hat +
  beta_DrLoganLevkoff_hat*DrLoganLevkoff_grid +
  beta_DrPepperSchwartz_hat + beta_Married..1..Divorced..0._hat

lines(DrLoganLevkoff_grid, reg_line_Married..1..Divorced..0.,
      lwd = 3, col = 'red')
...
##################################################
# End
>>>>>>> d6e029c7c8003ad927e39c1e1727e8ce417b2a40
##################################################